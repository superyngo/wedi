...
| B2 | No test checked that the help panel, `KEYMAP.md` and `handle_key_event` agree. `src/help.rs` `mod tests` maps each documented label to keys (explicit table; unknown labels fail), requires every documented key to be bound, both sources to list the same keys, and every bound shortcut to be in the help panel. It found two keys missing from the panel, now added | `test: help panel, KEYMAP.md and bindings stay in sync` (2026-09-24) |
| B17 | `Editor` owned `Terminal` and the dialogs read crossterm events inline, so command dispatch and dialog keys had no tests. Headless harness (`Editor::with_terminal` + `Terminal::with_size`, `src/editor.rs` `mod tests`) covers B3–B11; `src/dialog.rs` moves dialog state out of the I/O loops (`LineInput`, `confirm_answer`, `HelpPanelState`, one `next_event` reader) with unit tests. Verified on the real binary: CJK + paste in Ctrl+G (cursor column 16), confirm Y/n/Enter, help End and tab switch; fixed a `\r` paste into prompts found there. Not covered: `handle_command` arms that open dialogs (would need scripted dialogs) | `fix: stale selection no longer crashes copy/cut` (harness), `refactor: dialog state out of the event loops, unit-tested` (2026-09-24) |
