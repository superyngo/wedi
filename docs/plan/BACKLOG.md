...
| B17 | `Editor` owned `Terminal` and the dialogs read crossterm events inline, so command dispatch and dialog keys had no tests. Headless harness (`Editor::with_terminal` + `Terminal::with_size`, `src/editor.rs` `mod tests`) covers B3–B11; `src/dialog.rs` moves dialog state out of the I/O loops (`LineInput`, `confirm_answer`, `HelpPanelState`, one `next_event` reader) with unit tests. Verified on the real binary: CJK + paste in Ctrl+G (cursor column 16), confirm Y/n/Enter, help End and tab switch; fixed a `\r` paste into prompts found there. Not covered: `handle_command` arms that open dialogs (would need scripted dialogs) | `test: headless editor harness` (earlier), `refactor: dialog state out of the event loops, unit-tested` (2026-09-24) |
