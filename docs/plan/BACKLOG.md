...
| B1 | CI was disabled. `.github/workflows/ci.yml` runs fmt on Ubuntu, and clippy `-D warnings` (default and no default features) plus `cargo test --workspace` on Ubuntu, Windows and macOS, on push/PR to `main` | `ci: re-enable fmt, clippy and tests on push to main` (2026-09-24) |
