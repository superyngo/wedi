...
| B14 | 2026-09-24 | 2026-09-24 | P1 | `cargo clippy -- -D warnings` fails (`items_after_test_module`); `--no-default-features` warns on unneeded `mut` | audit F16; `rope_buffer.rs` `impl Default for RopeBuffer`; `help::get_help_sections` | XS | Clippy with `-D warnings` passes with and without default features |
