...
impl RopeBuffer {
    pub fn new() -> Self {
        ...
        crate::debug_log!("RopeBuffer::new()");
        crate::debug_log!("  System default encoding: {}", system_enc.name());
    ...
    pub fn get_system_ansi_encoding() -> &'static encoding_rs::Encoding {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            crate::debug_log!("Detecting system encoding on Windows:");
            crate::debug_log!("  Console Input CP (GetConsoleCP): {}", console_input_cp);
            crate::debug_log!(
                "  Console Output CP (GetConsoleOutputCP): {}",
                console_output_cp
            );
            crate::debug_log!("  System ANSI CP (GetACP): {}", system_acp);

            // 優先使用控制台輸出代碼頁，如果是 0 則回退到系統 ANSI 代碼頁
            let cp = if console_output_cp != 0 {
                crate::debug_log!("  Using Console Output CP: {}", console_output_cp);
                console_output_cp
            } else {
                crate::debug_log!("  Console CP is 0, using System ANSI CP: {}", system_acp);
                system_acp
            };

            let encoding = match cp {
                65001 => {
                    crate::debug_log!("  Using UTF-8 (CP 65001)");
                    encoding_rs::UTF_8
                }
                936 => {
                    crate::debug_log!("  Using GBK (CP 936)");
                    encoding_rs::GBK
                }
                950 => {
                    // 中文(繁體) - Big5
                    crate::debug_log!("  Using Big5 (CP 950)");
                ...
                932 => {
                    crate::debug_log!("  Using Shift_JIS (CP 932)");
                    encoding_rs::SHIFT_JIS
                }
                949 => {
                    // 韓文 - EUC-KR
                    crate::debug_log!("  Using EUC-KR (CP 949)");
                ...
                1252 => {
                    crate::debug_log!("  Using Windows-1252 (CP 1252)");
                    encoding_rs::WINDOWS_1252
                }
                _ => {
                    crate::debug_log!("  Unknown code page, using UTF-8 as fallback");
                    encoding_rs::UTF_8
                }
            };
        ...
        #[cfg(not(target_os = "windows"))]
        {
            ...
            crate::debug_log!("Detecting system encoding on Unix-like system:");

            for var in &locale_vars {
                if let Ok(locale) = env::var(var) {
                    crate::debug_log!("  {} = {}", var, locale);
                    ...
                    if let Some(charset_start) = locale.find('.') {
                        ...
                        crate::debug_log!("  Detected charset: {}", charset);

                        match charset.to_uppercase().as_str() {
                            "UTF-8" => {
                                crate::debug_log!("  Using UTF-8");
                                return encoding_rs::UTF_8;
                            }
                            "GBK" | "GB2312" | "GB18030" => {
                                crate::debug_log!("  Using GBK");
                                return encoding_rs::GBK;
                            }
                            "BIG5" => {
                                if let Some(enc) = encoding_rs::Encoding::for_label(b"big5") {
                                    crate::debug_log!("  Using Big5");
                                    return enc;
                                }
                            }
                            "SHIFT_JIS" | "SJIS" => {
                                crate::debug_log!("  Using Shift_JIS");
                                return encoding_rs::SHIFT_JIS;
                            }
                            "EUC-KR" => {
                                if let Some(enc) = encoding_rs::Encoding::for_label(b"euc-kr") {
                                    crate::debug_log!("  Using EUC-KR");
                                    return enc;
                                }
                            }
                            _ => {
                                crate::debug_log!("  Unknown charset, continuing...");
                            } // 繼續檢查其他變數
                        }
                    }
                }
            ...
            crate::debug_log!("  No valid locale found, using UTF-8 as fallback");
            encoding_rs::UTF_8
        }
    }
    ...
    pub fn from_file_with_encoding(path: &Path, encoding_config: &EncodingConfig) -> Result<Self> {
        ...
        let (rope, detected_encoding, modified, bom, lossy) = if path.exists() {
            ...
            crate::debug_log!("  File does not exist, creating new buffer");
            if encoding_config.read_encoding.is_some() {
                crate::debug_log!(
                    "  Using user-specified encoding: {}",
    ...
    pub fn save(&mut self) -> Result<()> {
        if let Some(path) = &self.file_path.clone() {
            crate::debug_log!("Saving file: {}", path.display());
            crate::debug_log!("  save_encoding: {}", self.save_encoding.name());
